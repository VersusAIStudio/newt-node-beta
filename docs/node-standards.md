...
- GPT Image 2.5 uses variable token billing; Image 2's per-image table is not a valid estimate for it. Run buttons show `Variable cost`, history remains unpriced, and Newt's budget guard rejects unpriced automated generation unless the user explicitly enables Allow Unpriced Generations. Monitor both variants through the existing weekly pricing sources without treating a single token rate as a fixed per-image charge.
...
- Atlas's shared estimators in `src/atlasPricing.js` return cost objects with nullable `amountUsd`, `pricingSource`, and `pricingBasis`. The existing refresh service is wired with `enableAtlasPricing: true`; weekly and manual checks GET the [public catalog](https://api.atlascloud.ai/api/v1/pricing/models) without authentication or paid requests. Apply only verified standard `official_price` rows, never promotional/account discounts, generic starting prices, or token estimates as fixed charges. Validate route IDs, rules, dimensions, units, and price versions; preserve snapshots and historical amounts. Seedance 2.5 native 480p/720p/1080p requests without reference video use published per-second standard estimates for 4-30 seconds, explicitly marked `standard-estimate` and `token_postpaid`, never guaranteed quotes. Validate the separate reference route's `runtime_estimate` contract. Reference-video billing, super-resolution tiers, OpenAI image token billing, Seedance 2.0, absent REVE rates, Pro 2K, and other unverified settings remain variable/unknown, not zero. No separate Atlas pricing store.
